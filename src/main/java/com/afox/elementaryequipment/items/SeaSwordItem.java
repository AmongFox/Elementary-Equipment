...
import com.afox.elementaryequipment.ability.active.SeaSwordActiveAbility;
import com.afox.elementaryequipment.ability.passive.SeaSwordPassiveAbility;
...
public class SeaSwordItem extends SwordItem {
    static ModConfig.ConfigData.SeaSword config = ModConfig.getConfig().seaSword;
    ...
    /**
     * ЛКМ (пассив):
     * При ударе, увеличивает скорость плавания, даёт дышать под водой и накладывает отравление на противника.
     * Перезарядка: 20 секунд.
     * Износ: 1hp
     * ---
     * ПКМ (актив):
     * Позволяет совершить рывок под водой.
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (ABILITY_PASSIVE_COOLDOWN.checkCooldown(attacker.getWorld()) && attacker.isTouchingWater()) {
            SeaSwordPassiveAbility.execute(target, attacker);
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        if (player.isTouchingWater()) {
            if (ExperienceUtils.take(world, player, XP_COST)) {
                SeaSwordActiveAbility.execute(world, player);
